...
export function sortBundleDomains(
  ...
  if (
    ...
    sorted['levels'] = Object.entries(
      sorted['levels'] as Record<string, unknown>,
    )
  ...
  if (sorted['enemyCatalog'] && typeof sorted['enemyCatalog'] === 'object') {
    const entries = (sorted['enemyCatalog'] as { entries?: EnemyEntry[] })
      .entries;
  ...
  if (sorted['heroCatalog'] && typeof sorted['heroCatalog'] === 'object') {
    const entries = (sorted['heroCatalog'] as { entries?: HeroEntry[] })
      .entries;
    if (Array.isArray(entries)) {
  ...
  if (sorted['ammoCatalog'] && typeof sorted['ammoCatalog'] === 'object') {
    const entries = (sorted['ammoCatalog'] as { entries?: AmmoEntry[] })
      .entries;
    if (Array.isArray(entries)) {
  ...
  if (
    sorted['formationLayouts'] &&
    typeof sorted['formationLayouts'] === 'object'
  ) {
