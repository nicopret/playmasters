...
describe('SpaceBlaster catalog schemas', () => {
  it('accepts valid hero catalog', () => {
    const result = validateSchema(
      'HeroCatalog',
      SpaceBlasterSchema.heroCatalog,
      heroValid,
    );
  ...
  it('rejects hero catalog missing heroId', () => {
    ...
    expect(result.issues.some((i: Issue) => i.message.includes('heroId'))).toBe(
      true,
    );
  });
  ...
  it('rejects hero catalog with non-string spriteKey', () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.issues.some((i: Issue) => i.path.includes('entries.0.spriteKey')),
    ).toBe(true);
  });
  ...
  it('accepts valid enemy catalog', () => {
    const result = validateSchema(
      'EnemyCatalog',
      SpaceBlasterSchema.enemyCatalog,
      enemyValid,
    );
    expect(result.valid).toBe(true);
    expect(result.issues).toHaveLength(0);
  ...
  it('rejects enemy catalog missing enemyId', () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.issues.some((i: Issue) => i.message.includes('enemyId')),
    ).toBe(true);
  });
  ...
  it('rejects enemy catalog with hp <= 0', () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.issues.some((i: Issue) => i.path.includes('entries.0.hp')),
    ).toBe(true);
  });
  ...
  it('accepts valid ammo catalog', () => {
    const result = validateSchema(
      'AmmoCatalog',
      SpaceBlasterSchema.ammoCatalog,
      ammoValid,
    );
    expect(result.valid).toBe(true);
    expect(result.issues).toHaveLength(0);
  ...
  it('rejects ammo catalog with negative cooldown', () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.issues.some((i: Issue) =>
        i.path.includes('entries.0.fireCooldownMs'),
      ),
    ).toBe(true);
  });
  ...
  it('rejects ammo catalog with negative speed', () => {
    ...
    expect(
      result.issues.some((i: Issue) =>
        i.path.includes('entries.0.projectileSpeed'),
      ),
    ).toBe(true);
  });
});
