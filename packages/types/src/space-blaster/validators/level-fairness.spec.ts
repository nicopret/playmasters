...
describe('validateLevelFairness', () => {
  ...
  it('fails when probability < 0', () => {
    const issues = validateLevelFairness({ ...baseLevel, diveProbability: -0.1 });
    expect(issues.some((i: ValidationIssue) => i.path === 'diveProbability')).toBe(true);
  });
  ...
  it('fails when probability > 1 (wave-level)', () => {
    ...
    expect(
      issues.some((i: ValidationIssue) => i.path === 'waves[0].shootProbability')
    ).toBe(true);
  });

  it('fails when caps are negative', () => {
    const issues = validateLevelFairness({ ...baseLevel, maxConcurrentDivers: -1 });
    expect(issues.some((i: ValidationIssue) => i.path === 'maxConcurrentDivers')).toBe(true);
  });
  ...
  it('fails when turnRate exceeds cap while tracking enabled', () => {
    ...
    expect(issues.some((i: ValidationIssue) => i.path === 'turnRate')).toBe(true);
  });
