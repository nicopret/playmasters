...
describe('validateLevelStructure', () => {
  it('passes a valid level', () => {
    const issues = validateLevelStructure({ ...validLevel, levelId: 'level-valid' });
    expect(issues).toHaveLength(0);
  });

  it('fails when waves array is empty', () => {
    const issues = validateLevelStructure({ ...validLevel, levelId: 'no-waves', waves: [] });
  ...
  it('fails when wave count is negative', () => {
    ...
    expect(issues.some((i: ValidationIssue) => i.path === 'waves[0].count')).toBe(true);
  });
  ...
  it('fails when wave total enemies is zero', () => {
    ...
    expect(issues.some((i: ValidationIssue) => i.message.includes('must be > 0'))).toBe(true);
  });
  ...
  it('fails when bossWaveIndex is out of range', () => {
    ...
    expect(issues.some((i: ValidationIssue) => i.path === 'boss.bossWaveIndex')).toBe(true);
  });
