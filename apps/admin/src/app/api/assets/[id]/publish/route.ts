...
export async function POST(
  ...
  try {
    ...
    if (
      code === 'no_draft' ||
      code === 'version_not_current_draft' ||
      code === 'version_not_draft'
    )
      return bad('invalid_draft', 400);
    if (code === 'invalid_asset_type') return bad('invalid_asset_type', 400);
    if (code === 'background_too_large')
      return bad('background_too_large', 400);
    if (code === 'background_invalid_format')
      return bad('background_invalid_format', 400);
