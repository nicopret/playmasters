...
export async function POST(
  ...
  const changeNotes =
    typeof body.changeNotes === 'string' ? body.changeNotes : undefined;
  ...
  try {
    ...
    if (code === 'no_published_version')
      return bad('no_published_version', 400);
    if (code === 'draft_already_exists')
      return bad('draft_already_exists', 409);
    if (
      code === 'published_version_not_found' ||
      code === 'version_not_published'
    )
      return bad('published_version_missing', 400);
    if (code === 'draft_or_published_bucket_missing')
      return bad('storage_not_configured', 500);
